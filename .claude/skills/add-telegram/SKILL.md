...
This skill adds Telegram support to MatterBot. Users can choose to:
...
MatterBot uses a **Channel abstraction** (`Channel` interface in `src/types.ts`). Each messaging platform implements this interface. Key files:
...
>
> Check logs: `tail -f logs/matterbot.log`
...
4. Service is running: `launchctl list | grep matterbot`
...
- Check bot is started: `tail -f logs/matterbot.log`
...
8. Rebuild: `npm run build && launchctl kickstart -k gui/$(id -u)/com.matterbot`
