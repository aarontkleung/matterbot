...
\`\`\`bash
docker images | grep matterbot-agent
echo '{}' | docker run -i --entrypoint /bin/echo matterbot-agent:latest "Container OK" || echo "Container build failed"
\`\`\`
```
...
```bash
echo '{}' | docker run -i --entrypoint /bin/echo matterbot-agent:latest "Container OK"
```
...
docker run --rm --entrypoint /bin/bash -v /tmp/test-ro:/test:ro matterbot-agent:latest \
...
```bash
mkdir -p /tmp/test-rw
docker run --rm --entrypoint /bin/bash -v /tmp/test-rw:/test matterbot-agent:latest \
