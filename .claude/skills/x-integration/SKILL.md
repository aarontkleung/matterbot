---
name: x-integration
description: X (Twitter) integration for MatterBot. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** MatterBot v1.0.0. Directory structure may change in future versions.
...
1. **MatterBot is installed and running** - WhatsApp connected, service active
...
# Verify: launchctl list | grep matterbot shows PID and exit code 0
```
...
| `MATTERBOT_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/matterbot.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into MatterBot, make the following modifications:
...
All paths below are relative to project root (`MATTERBOT_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep matterbot  # Should show PID and exit code 0 or -
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/matterbot.log | tail -20

# Script errors
grep -i "error\|failed" logs/matterbot.log | tail -20
```
...
container run matterbot-agent ls -la /app/src/skills/
```
