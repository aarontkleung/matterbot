...
|-----|----------|---------|
| **Main app logs** | `logs/matterbot.log` | Host-side WhatsApp, routing, container spawning |
| **Main app errors** | `logs/matterbot.error.log` | Host-side errors |
...
**Workaround:** The system extracts only authentication variables (`CLAUDE_CODE_OAUTH_TOKEN`, `ANTHROPIC_API_KEY`) from `.env` and mounts them for sourcing inside the container. Other env vars are not exposed. This keeps credentials out of process listings.

To verify env vars are reaching the container:
```bash
echo '{}' | docker run -i \
  -v $(pwd)/data/env:/workspace/env-dir:ro \
  --entrypoint /bin/bash matterbot-agent:latest \
...
```bash
docker run --rm --entrypoint /bin/bash matterbot-agent:latest -c 'ls -la /workspace/'
```
...
```bash
docker run --rm --entrypoint /bin/bash matterbot-agent:latest -c '
  whoami
  ls -la /workspace/
  ls -la /app/
'
...
```bash
docker run --rm -it --entrypoint /bin/bash matterbot-agent:latest
```
...
# Also clear the session ID from MatterBot's tracking (stored in SQLite)
sqlite3 store/messages.db "DELETE FROM sessions WHERE group_folder = '{groupFolder}'"
```

To verify session resumption is working, check the logs for the same session ID across messages:
```bash
grep "Session initialized" logs/matterbot.log | tail -5
...
```bash
echo "=== Checking MatterBot Container Setup ==="
...
echo '{}' | docker run -i --entrypoint /bin/echo matterbot-agent:latest "OK" 2>/dev/null || echo "MISSING - run ./container/build.sh"
...
SESSIONS=$(grep "Session initialized" logs/matterbot.log 2>/dev/null | tail -5 | awk '{print $NF}' | sort -u | wc -l)
