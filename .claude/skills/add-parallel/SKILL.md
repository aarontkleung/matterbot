...
Adds Parallel AI MCP integration to MatterBot for advanced web research capabilities.
...
- **Non-blocking Design** - Uses MatterBot scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. MatterBot already set up and running
...
Add Parallel AI MCP servers after the matterbot server:
```typescript
const mcpServers: Record<string, any> = {
  matterbot: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__matterbot__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__matterbot__send_message
   4. Use mcp__matterbot__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | container run -i --entrypoint /bin/echo matterbot-agent:latest "Container OK"
```
...
- Check task runs: `tail -f logs/matterbot.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.matterbot`
