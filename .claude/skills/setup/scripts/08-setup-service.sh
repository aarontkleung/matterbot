...
case "$PLATFORM" in
  ...
  macos)
    PLIST_PATH="$HOME_PATH/Library/LaunchAgents/com.matterbot.plist"
...
<dict>
    ...
    <string>${PROJECT_PATH}/logs/matterbot.log</string>
    <key>StandardErrorPath</key>
    <string>${PROJECT_PATH}/logs/matterbot.error.log</string>
</dict>
</plist>
PLISTEOF
    ...
    if launchctl list 2>/dev/null | grep -q "com.matterbot"; then
      SERVICE_LOADED="true"
...
EOF
  ...
  linux)
    ...
    UNIT_PATH="$UNIT_DIR/matterbot.service"
    mkdir -p "$UNIT_DIR"
    log "Generating systemd unit at $UNIT_PATH"

    cat > "$UNIT_PATH" <<UNITEOF
[Unit]
Description=MatterBot Personal Assistant
...
StandardOutput=append:${PROJECT_PATH}/logs/matterbot.log
StandardError=append:${PROJECT_PATH}/logs/matterbot.error.log
...
UNITEOF
    ...
    systemctl --user enable matterbot >> "$LOG_FILE" 2>&1 || true
    systemctl --user start matterbot >> "$LOG_FILE" 2>&1 || true

    # Verify
    SERVICE_LOADED="false"
    if systemctl --user is-active matterbot >/dev/null 2>&1; then
      SERVICE_LOADED="true"
