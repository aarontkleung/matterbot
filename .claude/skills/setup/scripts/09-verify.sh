...
if [ "$PLATFORM" = "macos" ]; then
  if launchctl list 2>/dev/null | grep -q "com.matterbot"; then
    ...
    LAUNCHCTL_LINE=$(launchctl list 2>/dev/null | grep "com.matterbot" || true)
  ...
  if systemctl --user is-active matterbot >/dev/null 2>&1; then
    SERVICE="running"
  elif systemctl --user list-unit-files 2>/dev/null | grep -q "matterbot"; then
    SERVICE="stopped"
  fi
fi
log "Service: $SERVICE"
...
if [ -f "$HOME/.config/matterbot/mount-allowlist.json" ]; then
