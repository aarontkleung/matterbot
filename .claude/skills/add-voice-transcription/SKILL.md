...
description: Add voice message transcription to MatterBot using OpenAI's Whisper API. Automatically transcribes WhatsApp voice notes so the agent can read and respond to them.
---
...
Then install it. **IMPORTANT:** The OpenAI SDK requires Zod v3 as an optional peer dependency, but MatterBot uses Zod v4. This conflict is guaranteed, so always use `--legacy-peer-deps`:
...
Find the orphan cleanup block in `ensureContainerSystemRunning()` (the section starting with `// Kill and clean up orphaned MatterBot containers from previous runs`) and replace it with:
...
```typescript
  ...
  try {
    ...
    const matterbotContainers = containers.filter(
      (c) => c.configuration.id.startsWith('matterbot-'),
    );
    const running = matterbotContainers
    ...
    const allNames = matterbotContainers.map((c) => c.configuration.id);
...
nc = [c['configuration']['id'] for c in data if c['configuration']['id'].startswith('matterbot-')]
if nc: print(' '.join(nc))
...
```bash
launchctl kickstart -k gui/$(id -u)/com.matterbot
```

Verify it started with exactly one (or zero, before first message) matterbot container:

```bash
sleep 3 && launchctl list | grep matterbot
container ls -a --format json | python3 -c "
import sys, json
data = json.load(sys.stdin)
nc = [c for c in data if c['configuration']['id'].startswith('matterbot-')]
print(f'{len(nc)} matterbot container(s)')
...
```bash
tail -f logs/matterbot.log | grep -i "voice\|transcri"
```
...
```bash
# Stop all matterbot containers, then restart the service
container ls -a --format json | python3 -c "
import sys, json
data = json.load(sys.stdin)
running = [c['configuration']['id'] for c in data if c['configuration']['id'].startswith('matterbot-') and c['status'] == 'running']
...
```bash
tail -100 logs/matterbot.log | grep -i transcription
```
...
The OpenAI SDK requires Zod v3, but MatterBot uses Zod v4. This conflict is guaranteed — always use:
```bash
npm install --legacy-peer-deps
```
